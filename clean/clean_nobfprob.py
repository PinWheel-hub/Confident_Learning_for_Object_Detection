    ...
    with open("clean/clean_msg.txt", "a") as f, open("clean/clean_msg_fb.txt", "a") as f_fb:
        ...
        for i, det_result in enumerate(det_results):
            if positives[i] == 0 and scores[i] > score_thr:  # FN
                ...
                f.write(msg + '\n')
                if det_result[4:].max() < 0.05:
                    pred_prob = np.append(det_result[4:], 0)
                else:
                    pred_prob = np.append(det_result[4:], 1)
                pred_probs.append(pred_prob)
...
def main():
    ...
    with open('clean/my_wrong_b.txt', 'w') as f:
        for key in result:
